...
export default function PatientOnboarding() {
  ...
  const healthId = authUser.username
    ? `${authUser.username}@healthapp`
    : authUser.email || "";
    ...
    (async () => {
      try {
        ...
        if (me) {
          ...
          if (me.allergies) {
            // Put existing allergies into notes section by default
            setValue("allergyNotes", me.allergies);
          }
          if (me.emergencyContact) setValue("emergencyContact", me.emergencyContact);
          return;
        }

        // If no patient profile yet, use auth user as a friendly starting point
        if (authUser && !me) {
          if (!watch("firstName") && authUser.username) {
            // Simple default: put username into first name; user can update
  ...
  const onSubmit = async (form) => {
    try {
      ...
      const msg =
        e?.response?.data?.message || e?.message || "Unable to save profile. Please try again.";
      toast.error(msg);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div className="max-w-3xl">
          ...
          <p className="mt-2 text-sm sm:text-[15px] text-gray-600">
            We will use this information to help your care team treat you safely. You can update it
        ...
        {healthId && (
          <div className="bg-sky-50 border border-sky-200 rounded-xl px-4 py-3 text-xs sm:text-sm text-sky-800">
            <p className="font-semibold">Your HealthApp ID</p>
            <p className="mt-1 font-mono text-[12px] break-all">{healthId}</p>
          </div>
        )}
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              {errors.gender && (
                <p className="mt-1 text-xs text-rose-600">{errors.gender.message}</p>
              )}
            </div>
          ...
          <div>
            ...
            <Input placeholder="Enter your phone number" {...register("phoneNumber")} />
            {errors.phoneNumber && (
          ...
          <div className="flex items-center justify-between pt-2 gap-4 flex-wrap">
            <p className="text-xs text-gray-500 max-w-md">
              By continuing, you confirm that the above information is accurate to the best of your
              knowledge. You can update it later from your account settings.
            </p>
